...
def beam_search_ev(examples, config: Config, llm: LLM, prm: PRM):
    ...
    for p in problems:
        ...
        boxed_answers = [extract_boxed_answer(c) for c in completions]
        
        # Count occurrences of boxed answers
        answer_counts = Counter(boxed_answers)
        most_common = answer_counts.most_common()
        
        # Choose voted answer
        if most_common and most_common[0][0] is not None:
            top_answer, _ = most_common[0]
            # If multiple completions have this answer, pick the one with highest PRM score
            candidate_indices = [i for i, ans in enumerate(boxed_answers) if ans == top_answer]
            if len(candidate_indices) == 1:
                pred = completions[candidate_indices[0]]
            else:
                # Pick the one with highest score
                agg_scores = [
                    aggregate_scores(beams[i].all_scores, config.agg_strategy)
                    for i in candidate_indices
                ]
                best_index = candidate_indices[np.argmax(agg_scores)]
                pred = completions[best_index]
        else:
            # fallback to PRM-based selection
            agg_scores = [
